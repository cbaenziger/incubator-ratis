...
public class SegmentedRaftLogOutputStream implements Closeable {
  ...
  private static final RetryPolicy<Object> retryObjectPolicy = new RetryPolicy<Object>()
      .handle(IOException.class)
      .onRetry(e -> LOG.warn("Retrying operation:", e.getLastFailure()))
      .onFailure(e -> {LOG.error("Failed to operation:", e.getFailure()); throw(new IOException(e.getFailure()));})
  ...
  public SegmentedRaftLogOutputStream(File file, boolean append, long segmentMaxSize,
    ...
    try {
      Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{
        fc = rp.getChannel();
        fc.position(fc.size());
  ...
  public void write(LogEntryProto entry) throws IOException {
    ...
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{cout.writeUInt32NoTag(serialized);});
    ///XXX Nthis seems not idempotent!
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{
      entry.writeTo(cout);
    });
    ...
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{out.write(buf);});
    writeInt(sum);
  }

  private void writeInt(int v) throws IOException {
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{out.write((v >>> 24) & 0xFF);});
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{out.write((v >>> 16) & 0xFF);});
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{out.write((v >>>  8) & 0xFF);});
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{out.write((v) & 0xFF);});
  }

  private void create() throws IOException {
    Failsafe.with(retryObjectPolicy).run((CheckedRunnable)()->{fc.truncate(0);});
    fc.position(0);
    preallocatedPos = 0;
  ...
  public void close() throws IOException {
    try {
      Failsafe.with(retryObjectPolicy).run((CheckedRunnable)(out::flush));
